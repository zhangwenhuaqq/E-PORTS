...
from page.page_voyage_manager_total_add_receipt import PageVoyageManagerTotalAddReceipt
...
@pytest.mark.parametrize("indata", get_read(get_file_path('scripts_data', 'voyage_manager_total_add_receipt.yaml')))
def test_voyage_manager_operate_agent_Bill(driver, indata):
    ...
    # 调用航管-结算管理总单添加收款方法
    voyage_add_receipt = PageVoyageManagerTotalAddReceipt(driver)
    voyage_add_receipt.voyage_Manager_Total_Add_Settlement_receipt(record,indata["Collection_Serial_Number"],indata["Voucher_flow_amount"],indata["Name_receiver"])
    if indata["success"]:
        try:
            # 判断添加收款按钮存在
            assert voyage_add_receipt.page_is_add_receipt_success()
            log.info("航管-结算管理总单添加收款成功")
